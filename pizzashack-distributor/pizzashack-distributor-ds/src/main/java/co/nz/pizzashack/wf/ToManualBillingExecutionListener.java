...
@Component("toManualBillingExecutionListener")
public class ToManualBillingExecutionListener {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(ToManualBillingExecutionListener.class);

	public void execute(ExecutionListenerExecution execution) throws Exception {
		LOGGER.info("ToManualBillingExecutionListener start:{} ");
		...
		if (!indenpendentProcess) {
			// if current billing process is subprocess,
			// some flow variable need to be wrote back to its associated one in
			// main flow. so when flow paused, its variable can also be
			// retrieved by its parent instanceId
	...
	private void workForOrderProcess(ExecutionListenerExecution execution) {
		...
		BillingDto billingTrans = (BillingDto) parentExecutionEntity
				.getVariable(BILLING_SUB_PROCESS_OBJ);
		parentExecutionEntity.setVariable(BILLING_MAIN_PROCESS_OBJ,
				billingTrans);
		...
		orderProcess.getOrder().setStatus("pendingOnBilling");
